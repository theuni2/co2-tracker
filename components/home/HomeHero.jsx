...
export default function HomeHero() {
  return (
    <section className="relative w-full min-h-[88vh] flex flex-col justify-center items-center py-20 px-4 sm:px-6 text-center bg-slate-950 overflow-hidden border-b border-emerald-500/20 shadow-2xl">
      ...
      <div className="absolute left-6 sm:left-10 top-24 hidden xl:flex items-center space-x-2 px-3.5 py-2 bg-slate-900/90 border border-emerald-500/40 rounded-2xl backdrop-blur-md shadow-xl animate-float z-10">
        <span className="text-2xl">📚</span>
        <span className="text-xs font-bold text-emerald-300">Storybooks for Libraries</span>
      </div>

      <div className="absolute right-6 sm:right-10 top-24 hidden xl:flex items-center space-x-2 px-3.5 py-2 bg-slate-900/90 border border-teal-500/40 rounded-2xl backdrop-blur-md shadow-xl animate-float-reverse z-10">
        <span className="text-2xl">🎓</span>
      ...
      <div className="relative z-10 space-y-8 max-w-4xl mx-auto">
        ...
        {/* Quick Metrics Bar - Updated according to user request */}
        <div className="pt-8 max-w-4xl mx-auto grid grid-cols-2 sm:grid-cols-4 gap-4 border-t border-slate-800/80">
          {/* Box 1: Redistribute */}
          <div className="p-4 bg-slate-900/85 border border-slate-800/90 hover:border-emerald-500/40 rounded-2xl backdrop-blur-md transition-colors group">
            <p className="text-xl sm:text-2xl font-black text-white group-hover:text-emerald-400 transition-colors">
              Redistribute & ReCycle
            </p>
            {/* <p className="text-xs text-slate-400 font-medium mt-1">
              NGO Book & Notebook Reuse
            </p> */}
          </div>
          ...
          {/* Box 3: Emissions Saved */}
          <div className="p-4 bg-slate-900/85 border border-slate-800/90 hover:border-emerald-500/40 rounded-2xl backdrop-blur-md transition-colors group">
            <p className="text-xl sm:text-2xl font-black text-white group-hover:text-emerald-400 transition-colors">
              Emissions Saved
            </p>
            <p className="text-xs text-slate-400 font-medium mt-1">
              2.99 - 4.50 kg CO₂e / kg
            </p>
          </div>

          {/* Box 4: 100% Browser Private (Left as it is) */}
          <div className="p-4 bg-slate-900/85 border border-slate-800/90 hover:border-teal-500/40 rounded-2xl backdrop-blur-md transition-colors group">
