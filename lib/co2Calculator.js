import { RECYCLING_CO2_FACTORS, REUSE_CO2_FACTORS, PAPER_SHEET_WEIGHTS, PAPER_TYPES } from './co2Factors.js';
...
export function validateInputs({ method, sheets, sheetSize, weightKg, paperType }) {
  ...
  if (method === 'sheets') {
    if (sheets === '' || sheets === null || sheets === undefined) {
      return { isValid: false, error: 'Please enter the number of sheets.' };
    }
    const numSheets = Number(sheets);
    if (isNaN(numSheets)) {
...
/**
 * Calculates CO2e avoided for both Reuse and Recycle approaches.
 * @param {Object} params
 * @param {'reuse' | 'recycle'} [params.approachMode='reuse']
 ...
 * @returns {Object} Calculation result metadata
 */
export function calculateCO2Impact({ approachMode = 'reuse', method, sheets, sheetSize = 'A4', directWeightKg, paperType }) {
  ...
  const factorsTable = approachMode === 'reuse' ? REUSE_CO2_FACTORS : RECYCLING_CO2_FACTORS;
  const co2Factor = factorsTable[paperType] || RECYCLING_CO2_FACTORS[paperType] || 3.15;
